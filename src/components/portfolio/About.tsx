      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // Disconnect once fired
            observer.disconnect();
          }
        });
      },
      { threshold: 0.2 } // slightly lower threshold for better viewport triggers on smaller screens
    );
        ...
        <div className="col-span-12 md:col-span-9">
          ...
          <div className="space-y-6 max-w-3xl">
            <h2 className="font-display font-black text-[9vw] sm:text-[8vw] md:text-[4vw] tracking-[-0.06em] uppercase leading-none text-white">
              PITCHER&rsquo;S MIND.<br />
              <span className="text-outline">BUILDER&rsquo;S HANDS.</span>
            </h2>
            
            <p className="font-display text-sm md:text-base text-white/70 leading-relaxed max-w-2xl">
              Fresher with experience in BD, recruitment, and market research; international hackathon 
              champion (<span className="text-[#ffff00]">Rank 1 of 6,200+</span>) recognized for pitching product strategy across AgriTech, AR, 
          ...
          <div ref={statsRef} className="mt-16 grid grid-cols-2 md:grid-cols-4 border-t-2 border-white">
            {stats.map((stat, i) => (
              <div
                key={stat.label}
                className={`p-6 md:p-8 hover:bg-[#ffff00] hover:text-black transition-colors group ${
